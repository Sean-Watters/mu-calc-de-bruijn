{-# OPTIONS --safe #-}
module MuCalc.Thinning.Syntax.Substitution where
...
-- Renamings are maps of variables - which are thinnings!
Rename : ℕ → ℕ → Set
Rename n m = Thin n m

-- Extending a renaming `ρ` when we go under a binder:
-- Old way: `ext ρ`
-- New way; `inj ρ`

-- Incrementing all the variables by 1:
-- Old way: `rename F.suc`
-- New way: `rename pad`
...
rename ρ (var x) = var (inject x ρ) -- because renamings and variables are both thinnings, applying a renaming is thinning post-composition
rename ρ (μML₀ op) = μML₀ op
rename ρ (μML₁ op ϕ) = μML₁ op (rename ρ ϕ)
rename ρ (μML₂ op ϕ ψ) = μML₂ op (rename ρ ϕ) (rename ρ ψ)
rename ρ (μMLη op ϕ) = μMLη op (rename (inj ρ) ϕ)

-- Substitution extension
exts : ∀ {At n m} → Subst At n m → Subst At (suc n) (suc m)
exts σ fzero = var Fin.zero
exts σ (fsuc x) = rename inc (σ x)
